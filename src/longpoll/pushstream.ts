...
class PushStream {
  ...
  onmessage:
    | ((
        ...
        channel: string,
        eventid: string,
        isLastMessageFromBatch: boolean
      ) => void)
    | null
  ...
  constructor(settings: any) {
    ...
    this.reconnectOnChannelUnavailableInterval =
      settings.reconnectOnChannelUnavailableInterval || 60000
  ...
  private readonly _maxReconnectAttempts: number = 3
  _reconnect(timeout: number): void {
    // if (this._keepConnected && !this._reconnecttimer && this.readyState !== this.CONNECTING) {
    ...
    if (this._keepConnected && !this._reconnecttimer && this.readyState !== this.CONNECTING) {
      if (this._reconnectAttempts < this._maxReconnectAttempts) {
        ...
        console.log(`尝试重新连接，第 ${this._reconnectAttempts} 次，将在 ${timeout}ms 后重连`)
        this._reconnecttimer = window.setTimeout(() => {
          ...
          if (
            ...
            console.error('重新连接失败，已达到最大重试次数')
            this._onerror(new Error('重新连接失败，已达到最大重试次数'))
          }
        }, timeout)
      } else {
  ...
  _useControlArguments() {
    return (
      this.messagesControlByArgument && (this._lastModified !== null || this._lastEventId !== null)
    )
  }
